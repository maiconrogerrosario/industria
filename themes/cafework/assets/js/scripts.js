$(function () {
    ...
    $("[data-mobilemenu]").click(function (e) {
        ...
        if (action === 'open') {
            $(".cafecontrol_sidebar").slideDown(effecttime);
        }

        if (action === 'close') {
            $(".cafecontrol_sidebar").slideUp(effecttime);
        }
    });
    ...
    $("[data-modalopen]").click(function (e) {
        ...
        $(".cafecontrol_modal").fadeIn(effecttime).css("display", "flex");
   ...
   //DATA SET
    ...
    $("[data-onpaidexpense]").click(function (e) {
        var clicked = $(this);
        var dataset = clicked.data();
        $.post(clicked.data("onpaidexpense"), dataset, function (response) {
            //reload by error
            if (response.reload) {
                window.location.reload();
            }
			...
			//Balance
            $(".j_total_paid_expense").text("R$ " + response.onpaid.paid);
            $(".j_total_unpaid_expense").text("R$ " + response.onpaid.unpaid);
    ...
    $("[data-onpaidincome]").click(function (e) {
        var clicked = $(this);
        var dataset = clicked.data();
        $.post(clicked.data("onpaidincome"), dataset, function (response) {
            //reload by error
            if (response.reload) {
                window.location.reload();
            }
			...
			//Balance
            $(".j_total_paid_income").text("R$ " + response.onpaid.paid);
            $(".j_total_unpaid_income").text("R$ " + response.onpaid.unpaid);
     ...
     * APP ON STAGESTATUS
     */
    $("[data-stagestatus]").click(function (e) {
        var clicked = $(this);
        var dataset = clicked.data();

        $.post(clicked.data("stagestatus"), dataset, function (response) {
            //reload by error
            if (response.reload) {
                window.location.reload();
            }
			
			if (response.stagestatus.status == 'active'){
					var stagestatus = 'ATIVO';
					
			
			}else if (response.stagestatus.status == 'finished'){
					var stagestatus = 'INATIVO';
			
			}
			
            //Balance
            $(".j_stage_status").text(stagestatus);
			 //Balance
     ...
     * APP ON TASKSTATUS
     */
    $("[data-taskstatus]").click(function (e) {
        var clicked = $(this);
        var dataset = clicked.data();

        $.post(clicked.data("taskstatus"), dataset, function (callback) {
           
			if (callback.taskstatus) {
				$(".taskstatus").text(callback.taskstatus.status);
     ...
     * WALLET FILTER
     */
    $(".cafecontrol_header_widget .wallet").mouseenter(function () {
    ...
    $(".wallet_action").click(function () {
        $(this).parent().find(".wallet_overlay").fadeIn(200);
    });
